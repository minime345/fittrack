...
interface FilterOptions {
  dietCategories?: string[];  // например ["vegan", "keto"]
  maxCaloriesPerMeal?: number;
  mealTypes?: string[];       // например ["breakfast", "lunch", "dinner"]
}

export function filterMeals(options: FilterOptions): Meal[] {
  return meals.filter(meal => {
    // Филтър по диети (ако има подадени категории)
    if (options.dietCategories && options.dietCategories.length > 0) {
      // Искаме ястието да има ВСИЧКИ категории, посочени в dietCategories
      const matchesDiet = options.dietCategories.every(cat =>
        meal.categories.includes(cat)
      );
      if (!matchesDiet) return false;
    }

    // Филтър по калории (максимални калории за ястието)
    if (options.maxCaloriesPerMeal && meal.kcal > options.maxCaloriesPerMeal) {
      return false;
    }

    // Филтър по тип на ястието (например breakfast, lunch, dinner)
    if (options.mealTypes && options.mealTypes.length > 0) {
      if (!options.mealTypes.includes(meal.mealType)) {
        return false;
      }
    }
...
/**
 * Генерира 7-дневен хранителен режим с 3 ястия на ден (закуска, обяд, вечеря).
 * Използва филтър за диети и максимални калории на ястие, като се стреми да покрие
 * дневните калории (dailyKcalTarget) с разпределение към 3 хранения.
 * 
 * Ако няма ястие за някой тип, се слага null.
 */
export function generateWeeklyMealPlan(
  dailyKcalTarget: number,
  dietCategories: string[],
): MealPlan {
  // Разпределение на калориите за хранене (примерно: 30% - 40% - 30%)
  const breakfastKcal = dailyKcalTarget * 0.3;
  const lunchKcal = dailyKcalTarget * 0.4;
  const dinnerKcal = dailyKcalTarget * 0.3;

  const mealPlan: MealPlanDay[] = [];

  // Филтрираме ястия за всеки тип хранене според диетите и калории
  const breakfasts = filterMeals({
    dietCategories,
    maxCaloriesPerMeal: breakfastKcal,
    mealTypes: ["breakfast"],
  });

  const lunches = filterMeals({
    dietCategories,
    maxCaloriesPerMeal: lunchKcal,
    mealTypes: ["lunch"],
  });

  const dinners = filterMeals({
    dietCategories,
    maxCaloriesPerMeal: dinnerKcal,
    mealTypes: ["dinner"],
  });

  // За 7 дни
  for (let i = 0; i < 7; i++) {
    // Вземаме на случаен принцип ястия от филтрираните списъци (ако има)
  ...
  return {
    days: mealPlan,
    totalKcalPerDay: dailyKcalTarget,
  };
}
